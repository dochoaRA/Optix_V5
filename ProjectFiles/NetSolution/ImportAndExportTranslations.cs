...
public class ImportAndExportTranslations : BaseNetLogic
{
    ...
    public void ImportTranslations()
    {
        ...
        bool wrapFields = GetWrapFields();
        // MO: Getdictionary() can read more lines in the csv (e.g. 10179) then there is data (e.g. 10159). As long as you do not get any errors of missing keys etc, everything is fine
        var localizationDictionary = GetDictionary();
        if (localizationDictionary == null)
        {
        ...
        try
        {
            using (var csvReader = new CSVFileReader(csvPath) { FieldDelimiter = characterSeparator.Value, WrapFields = wrapFields })
            {
                ...
                if (fileTranslations.Count > 1)
                {
                    //Necessary formatting because the export of the inpackt file can not be read by Optix
                    // 1. Delete the first row
                    fileTranslations.RemoveAt(0);

                    // 2. Replace all entries in the first row (except for the first entry) with the correct language code using regex
                    var languageCodeMap = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
                    {
                        { "de", "de-DE" }, { "en", "en-US" }, { "li", "de-LI" }, { "pl", "pl-PL" }, { "nl", "nl-NL" },
                        { "da", "da-DK" }, { "es", "es-ES" }, { "it", "it-IT" }, { "fr", "fr-FR" }, { "zh", "zh-CN" },
                        { "hu", "hu-HU" }, { "pt", "pt-PT" }, { "ru", "ru-RU" }, { "ro", "ro-RO" }, { "sv", "sv-SE" },
                        { "fi", "fi-FI" }, { "no", "nb-NO" }, { "tr", "tr-TR" }, { "bg", "bg-BG" }, { "el", "el-GR" },
                        { "hr", "hr-HR" }, { "cs", "cs-CZ" }, { "ja", "ja-JP" }, { "id", "id-ID" }, { "sl", "sl-SI" },
                        { "et", "et-EE" }, { "he", "he-IL" }, { "sr", "sr-RS" }, { "sk", "sk-SK" }, { "pt-BR", "pt-BR" }
                    };

                    var regex = new System.Text.RegularExpressions.Regex(@"\(([^)]+)\)");
                    var firstRow = fileTranslations[0];
                    for (int i = 1; i < firstRow.Count; i++)
                    {
                        var value = firstRow[i];

                        // Special case for "Beschreibung"
                        if (value.Trim().Equals("Beschreibung", StringComparison.OrdinalIgnoreCase))
                        {
                            firstRow[i] = "de-LI";
                            continue;
                        }
                        ...
                        if (match.Success)
                        {
                            ...
                            if (languageCodeMap.TryGetValue(code, out string mappedCode))
                            {
                                firstRow[i] = mappedCode;
                            }
                            else
                            {
                                firstRow[i] = code; // fallback: use code as is
                            }
                        }
                        // else: leave as is if no brackets found
                    }
                }

                // Find the index of the "en-US" column
                int enUSIndex = fileTranslations[0].FindIndex(header => header.Trim().Equals("en-US", StringComparison.OrdinalIgnoreCase));
                if (enUSIndex >= 0)
                {
                    // Capitalize the first letter for all entries in the "en-US" column (skip header row)
                    for (int i = 1; i < fileTranslations.Count; i++)
                    {
                        var entry = fileTranslations[i][enUSIndex];
                        if (!string.IsNullOrEmpty(entry))
                        {
                            fileTranslations[i][enUSIndex] = char.ToUpper(entry[0]) + entry.Substring(1);
                        }
                    }
                }

                int deDEIndex = fileTranslations[0].FindIndex(header => header.Trim().Equals("de-DE", StringComparison.OrdinalIgnoreCase));
                if (deDEIndex >= 0)
                {
                    // Capitalize the first letter for all entries in the "en-US" column (skip header row)
                    for (int i = 1; i < fileTranslations.Count; i++)
                    {
                        var entry = fileTranslations[i][deDEIndex];
                        if (!string.IsNullOrEmpty(entry))
                        {
                            fileTranslations[i][deDEIndex] = char.ToUpper(entry[0]) + entry.Substring(1);
                        }
                    }
                }
    ...
    private string ReplaceSymbolWithNewLine(string i)
    {
        if (i == null) return null;
        string result = i.Replace(newLinePlaceHolder, "\n").Replace("%%NEWLINE", "\n");
        // Hardcoded replacements for %xxx and %0
        result = result.Replace("%xxx", "{#Placeholder}").Replace("%yyy", "{#Placeholder2}").Replace("%0", "{#Placeholder}");
        return result;
    }
